        ...
        {tab==="dashboard"&&<div>
          ...
          <Card style={{marginBottom:14}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9}}>
              ...
              {totalQ>0 && (
                totalM===0&&totalC===0
                ? <div style={{padding:"10px 13px",background:S.muted+"08",border:"1px solid "+S.border,borderRadius:9}}><div style={{fontSize:11,color:S.muted}}>○ Brak obecności — marka niewidoczna dla AI. Start od entity building i structured data.</div></div>
                : totalM===0&&totalC>0
                ? <div style={{padding:"10px 13px",background:S.gold+"08",border:"1px solid "+S.gold+"22",borderRadius:9}}><div style={{fontSize:11,color:S.muted,lineHeight:1.55}}>! <strong style={{color:S.gold}}>Cytowana bez nazwy</strong> ({fmtN(totalC)} cytowań, 0 wzmianek) — AI ufa stronie ale nie kojarzy marki. Priorytet: entity building.</div></div>
                : (totalM>0&&totalC/totalM>8)
                ? <div style={{padding:"10px 13px",background:S.coral+"08",border:"1px solid "+S.coral+"22",borderRadius:9}}><div style={{fontSize:11,color:S.muted,lineHeight:1.55}}>! <strong style={{color:S.coral}}>Dysproporcja</strong> {fmtN(totalC)} cytowań vs {fmtN(totalM)} wzmianek — anchor texty z nazwą marki.</div></div>
                : (totalM>=5&&totalC>0&&totalC/totalM<0.15)
                ? <div style={{padding:"10px 13px",background:S.purple+"08",border:"1px solid "+S.purple+"22",borderRadius:9}}><div style={{fontSize:11,color:S.muted,lineHeight:1.55}}>! <strong style={{color:S.purple}}>Wzmianki bez cytowań</strong> — AI zna markę ale nie poleca strony. Wdróż structured data.</div></div>
                : <div style={{padding:"10px 13px",background:S.green+"08",border:"1px solid "+S.green+"22",borderRadius:9}}><div style={{fontSize:11,color:S.muted,lineHeight:1.55}}>✓ <strong style={{color:S.green}}>Obecność potwierdzona:</strong> {fmtN(totalM)} wzmianek + {fmtN(totalC)} cytowań. Zwiększ przez content plan.</div></div>
              )}
              {allComps.length>0 && (
                (compCounts[allComps[0]]||0)>totalM*2
                ? <div style={{padding:"10px 13px",background:S.coral+"08",border:"1px solid "+S.coral+"22",borderRadius:9}}><div style={{fontSize:11,color:S.muted,lineHeight:1.55}}>⚔️ <strong style={{color:S.coral}}>{allComps[0]} dominuje</strong>: {fmtN(compCounts[allComps[0]]||0)} wzm vs {fmtN(totalM)} Twoich. Zbadaj ich content.</div></div>
                : (compCounts[allComps[0]]||0)>totalM
                ? <div style={{padding:"10px 13px",background:S.gold+"08",border:"1px solid "+S.gold+"22",borderRadius:9}}><div style={{fontSize:11,color:S.muted,lineHeight:1.55}}>⚔️ <strong style={{color:S.gold}}>{allComps[0]} nieznacznie wyprzedza</strong> ({fmtN(compCounts[allComps[0]]||0)} vs {fmtN(totalM)} wzm).</div></div>
                : <div style={{padding:"10px 13px",background:S.green+"08",border:"1px solid "+S.green+"22",borderRadius:9}}><div style={{fontSize:11,color:S.muted,lineHeight:1.55}}>⚔️ <strong style={{color:S.green}}>Marka wyprzedza {allComps[0]}</strong>. {fmtN(compCounts[allComps[0]]||0)} wzm vs Twoje {fmtN(totalM)}. Utrzymaj tempo.</div></div>
              )}
            </div>
          </Card>
