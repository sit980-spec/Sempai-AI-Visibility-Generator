...
/* ─── Ahrefs CSV parser (UTF-16, TAB-separated, multi-line cells) ──────────── */
function parseTSV(text) {
  // Split into rows respecting quoted multi-line cells
  const rows = [];
...
function detectPlatform(headers, rows) {
  // AI Overview has its own column name
  if (headers.includes("AI Overview")) return "ai_overview";
  // Others have a Model column
...
function parseAhrefsBuffer(buffer, brandDomain) {
  ...
  // Normalise brand domain: "gardenspace.pl" → "gardenspace"
  const brandKey = brandDomain
    .toLowerCase()
    .replace(/^https?:\/\/(www\.)?/, "")
    .replace(/\..*$/, "")
    .trim();
  ...
  const parsed = rows.map(r => {
    ...
    const volume      = parseInt(r[volIdx]) || 0;

    // mentioned = Mentions cell contains brand key
    const mentioned = brandKey ? mentionsRaw.includes(brandKey) : mentionsRaw.length > 0;
    // cited = Link URL contains brand domain
    const cited     = brandKey ? linkRaw.includes(brandKey) : false;

    // All other brand names in Mentions (competitors)
    const otherMentions = mentionsRaw
      .split(/[,\n]+/)
      .map(m => m.trim())
      .filter(m => m && (brandKey ? !m.includes(brandKey) : true));

    return { keyword, volume, mentioned, cited, otherMentions };
  }).filter(r => r.keyword);

  return { platformId: pid, rows: parsed };
}

/* ─── Aggregate proc data from parsed rows ─────────────────────────────────── */
function aggregatePlatform(parsedRows) {
  ...
  // Collect all competitors seen
  const compSet = {};
  parsedRows.forEach(r => {
    r.otherMentions.forEach(c => {
      compSet[c] = (compSet[c] || 0) + 1;
    });
  });
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      for(let i=0;i<nodes.length-1;i++){
        ...
        ctx.fillStyle=(a.hasData?S.green:"#1a3354")+"cc"; ctx.fill();
      }
      nodes.forEach(n=>{
        ...
        g.addColorStop(0,n.color+(n.hasData?"28":"08")); g.addColorStop(1,n.color+"04");
        ctx.beginPath(); ctx.arc(n.x,n.y,17,0,Math.PI*2);
        ctx.fillStyle=g; ctx.fill();
        ctx.strokeStyle=n.color+(n.hasData?"88":"33"); ctx.lineWidth=2; ctx.stroke();
...
function MultiDropZone({ onFiles }) {
  ...
  const handleFiles = files => {
    Array.from(files).forEach(file => {
      ...
      reader.onload = e => onFiles(file.name, e.target.result);
  ...
  return (
    <div
      ...
      style={{
        border:`2px dashed ${drag?S.green:S.border}`,borderRadius:14,
        padding:"36px 24px",cursor:"pointer",textAlign:"center",
        background:drag?S.green+"0a":"transparent",transition:"all .2s",
      }}>
      ...
      <div style={{fontSize:12,color:S.muted}}>
        Obsługiwane: AI Overview, ChatGPT, Gemini, Perplexity, Copilot — platforma wykrywana automatycznie
      </div>
    </div>
  );
}
...
.brand-name{font-size:19px;font-weight:800;color:#07111f}
...
<div class="header">
  <div class="logo"><div class="s">S</div><span class="brand-name">sempai</span><span class="badge">AI Visibility</span></div>
...
<div class="kpi-grid">
  <div class="kpi" style="border-top-color:#2edf8f"><div class="kl">AI Share of Voice</div><div class="kv" style="color:#2edf8f">${avgSOV}%</div><div class="ks">Średnia SOV platform</div><div class="pb-wrap"><div class="pb" style="width:${Math.min(avgSOV,100)}%;background:#2edf8f"></div></div></div>
  ...
  <div class="kpi" style="border-top-color:#f5c842"><div class="kl">Łączne zapytania</div><div class="kv" style="color:#f5c842">${totalQ.toLocaleString("pl-PL")}</div><div class="ks">${totalM} wzm. · ${totalC} cyt.</div></div>
</div>
...
<section>
  ...
  <table><thead><tr><th>Marka</th><th>Łączne wzmianki w AI</th><th>vs. ${brand.name||"Twoja marka"}</th></tr></thead>
  <tbody>
    <tr class="bench">
...
<section>
  ...
  <div class="ins-grid">
    ${best?`<div class="ins" style="background:#2edf8f08;border-color:#2edf8f2a"><span class="ins-icon" style="color:#1db872">↑</span><span class="ins-text"><strong>Najlepsza platforma:</strong> ${best.platform} z SOV ${best.sov}% — koncentrować działania na utrzymaniu tej pozycji.</span></div>`:""}
    ...
    ${visM>visC?`<div class="ins" style="background:#a78bfa08;border-color:#a78bfa2a"><span class="ins-icon" style="color:#a78bfa">!</span><span class="ins-text"><strong>Dysproporcja:</strong> Wzmianki ${visM}% vs cytowania ${visC}% — priorytet: structured data, E-E-A-T, link building.</span></div>`:`<div class="ins" style="background:#2edf8f08;border-color:#2edf8f2a"><span class="ins-icon" style="color:#1db872">✓</span><span class="ins-text"><strong>Zdrowa korelacja</strong> wzmianek (${visM}%) i cytowań (${visC}%) — dobry autorytet techniczny w oczach AI.</span></div>`}
    <div class="ins" style="background:#4da6ff08;border-color:#4da6ff2a"><span class="ins-icon" style="color:#4da6ff">◈</span><span class="ins-text"><strong>Zasięg AI:</strong> ${brand.name||"Marka"} widoczna w <strong>${PLATFORMS.filter(p=>proc[p.id]?.mentions>0).length}/${PLATFORMS.length}</strong> platformach.${PLATFORMS.filter(p=>proc[p.id]?.total>0&&proc[p.id]?.mentions===0).length>0?" Brak wzmianek mimo danych: "+PLATFORMS.filter(p=>proc[p.id]?.total>0&&proc[p.id]?.mentions===0).map(p=>p.short).join(", ")+".":""}</span></div>
    ${compRows.length>0&&compRows[0].mentions>totalM?`<div class="ins" style="background:#ff5c6a08;border-color:#ff5c6a2a"><span class="ins-icon" style="color:#e03050">⚠</span><span class="ins-text"><strong>Uwaga konkurencyjna:</strong> ${compRows[0].name} wyprzedza markę w liczbie wzmianek AI (${compRows[0].mentions} vs ${totalM}). Wskazane działania contentowe.</span></div>`:""}
    <div class="ins" style="background:#f5c84208;border-color:#f5c8422a"><span class="ins-icon" style="color:#d4a017">→</span><span class="ins-text"><strong>Strategia 3-6 mies.:</strong> FAQ i how-to pod zapytania z niską wzmiankowalnością, schema markup, budowanie wzmianek w zewnętrznych źródłach cytowanych przez AI.</span></div>
  </div>
</section>

<div class="footer">
  <div><div class="ft-name">sempai · Let us perform!</div><div class="ft-sub">sempai.pl · AI Visibility Report Generator</div></div>
  <div style="font-size:11px;color:#8899aa">Wygenerowano: ${date}</div>
</div>
</div></body></html>`;
}
...
export default function App() {
  ...
  // parsedData: { platformId → { total, mentions, citations, compSet } }
  ...
  const [rawBuffers, setRawBuffers] = useState({}); // filename→ArrayBuffer for re-parse

  const autoKey = brand.url.toLowerCase().replace(/^https?:\/\/(www\.)?/, "").replace(/\..*$/, "").trim() || brand.name.toLowerCase().split(/\s+/)[0];
  const brandKey = brandMentionKey.trim() || autoKey;

  // Re-parse all loaded files when brandKey changes
  useEffect(() => {
    if (!brandKey || Object.keys(rawBuffers).length === 0) return;
    const newParsed = {};
    Object.entries(rawBuffers).forEach(([fname, buf]) => {
      try {
        const result = parseAhrefsBuffer(buf, brandKey);
        if (result?.platformId) {
          newParsed[result.platformId] = aggregatePlatform(result.rows);
        }
      } catch(e) {}
    });
    if (Object.keys(newParsed).length > 0) setParsedData(newParsed);
  }, [brandKey]);
  ...
  const handleFiles = (filename, buffer) => {
    try {
      ...
      // Collect ALL unique mention values for brand picker
      const text = new TextDecoder("utf-16").decode(buffer);
      const allRows = parseTSV(text);
      if (allRows.length > 1) {
        const headers = allRows[0].map(h => h.replace(/^"|"$/g,"").trim());
        const mi = headers.indexOf("Mentions");
        if (mi >= 0) {
          const found = new Set();
      ...
      setErrors(e => [...e, `Błąd parsowania ${filename}: ${err.message}`]);
    }
  };

  // Build proc object (same shape as before)
  const proc = {};
  PLATFORMS.forEach(p => {
    proc[p.id] = parsedData[p.id] || { total:0, mentions:0, citations:0, compSet:{} };
  });

  // Collect all competitors across all platforms
  const compCounts = {};
  PLATFORMS.forEach(p => {
    const cs = proc[p.id]?.compSet || {};
    Object.entries(cs).forEach(([name, cnt]) => {
      compCounts[name] = (compCounts[name]||0) + cnt;
    });
  });
  const allComps = Object.entries(compCounts)
    .sort((a,b) => b[1]-a[1])
    .map(([name]) => name)
  ...
  const openReport = () => {
    const html = buildReport({brand,proc,sovData,totalQ,totalM,totalC,visM,visC,avgSOV,allComps,best,worst});
      ...
      <div style={{maxWidth:1000,margin:"0 auto",padding:"30px 28px 60px"}}>
          ...
          <div>
            ...
            <Card>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                {[
                  {icon:"📂",text:"Pliki CSV z Ahrefs w formacie UTF-16 — wgrywasz bezpośrednio bez konwersji"},
        ...
        {tab==="import"&&(
          <div>
            ...
            <p style={{fontSize:13,color:S.muted,marginBottom:20}}>
              Wgraj wszystkie pliki naraz — platforma wykrywana automatycznie. Brakująca platforma = wynik 0.
            </p>

            <MultiDropZone onFiles={handleFiles}/>

            {/* Brand picker — shown after files loaded */}
            {allDetectedMentions.length>0&&(
              <div style={{marginTop:16,padding:"16px 18px",background:"#0a1820",border:`1px solid ${totalM>0?S.green+"44":S.gold+"44"}`,borderRadius:12}}>
                <div style={{display:"flex",alignItems:"flex-start",gap:12}}>
                  ...
                  <div style={{flex:1}}>
                    {totalM>0?(
                      <div style={{fontSize:13,color:S.green,fontWeight:700,marginBottom:4}}>
                      ...
                      <>
                        ...
                        <div style={{fontSize:12,color:S.muted,marginBottom:10,lineHeight:1.6}}>
                          Marki znalezione w danych (kliknij aby wybrać swoją markę, lub wpisz ją ręcznie):
                        </div>
                          ...
                          {allDetectedMentions.map((m,i)=>{
                            ...
                            return (
                              <button key={i} onClick={()=>setBrandMentionKey(active?"":m)}
                                style={{padding:"4px 13px",borderRadius:20,fontSize:12,fontWeight:700,cursor:"pointer",
                                  background:active?cols[i%6]+"44":cols[i%6]+"1a",
                                  border:`1px solid ${active?cols[i%6]:cols[i%6]+"44"}`,
                                  color:cols[i%6]}}>
                                {m}
                              </button>
                            );
                        ...
                        {totalC>0&&<div style={{fontSize:11,color:S.sky,marginTop:8}}>💡 Uwaga: {totalC} cytowań znalezionych (Link URL zawiera domenę) — marka jest cytowana jako źródło, ale nie wymieniana z nazwy w odpowiedziach AI.</div>}
                      </>
                    )}
                    {totalM===0&&totalC===0&&<div style={{fontSize:11,color:S.muted,marginTop:4}}>Brak cytowań i wzmianek — sprawdź poprawność domeny w zakładce ① Klient.</div>}
                  </div>
                </div>
              </div>
            )}

            {/* Status załadowanych plików */}
              ...
              <Card style={{marginTop:20}}>
                ...
                <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                  {allComps.slice(0,12).map((c,i)=>{
                    const cols=[S.sky,S.coral,S.gold,S.purple,"#34d399",S.green];
                    return (
                      <span key={i} style={{padding:"4px 13px",borderRadius:20,fontSize:12,fontWeight:700,background:cols[i%6]+"1a",border:`1px solid ${cols[i%6]}44`,color:cols[i%6]}}>
                        {c} <span style={{opacity:.6,fontWeight:400}}>({compCounts[c]})</span>
                      </span>
                    );
                  })}
                </div>
        ...
        {tab==="dashboard"&&(
          <div>
            ...
            {allComps.length>0&&(
              <Card style={{marginBottom:14}}>
                <CLabel>Konkurenci — wzmianki w AI (auto-wykryte)</CLabel>
                <ResponsiveContainer width="100%" height={Math.min(40*allComps.slice(0,8).length+40,320)}>
                  <BarChart data={[{name:brand.name||"Twoja marka",count:totalM,fill:S.green},...allComps.slice(0,7).map((c,i)=>({name:c,count:compCounts[c],fill:[S.sky,S.coral,S.gold,S.purple,"#34d399",S.sky,S.coral][i]}))]} layout="vertical" margin={{top:8,right:40,left:10,bottom:0}}>
                    ...
                    <YAxis dataKey="name" type="category" tick={{fill:S.text,fontSize:11}} axisLine={false} tickLine={false} width={120}/>
                    <Tooltip content={<Tip/>}/>
                    <Bar dataKey="count" name="Wzmianki" radius={[0,4,4,0]} label={{position:"right",fill:S.muted,fontSize:10}}>
                      {[brand.name||"Twoja marka",...allComps.slice(0,7)].map((_,i)=>(
          ...
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:20}}>
              {[
                ...
                {n:"02",title:"Analiza konkurencji",desc:`${allComps.length>0?`${allComps.length} konkurentów wykrytych automatycznie z danych AI`:"Brak danych — wgraj pliki CSV"}`,color:S.sky},
                {n:"★", title:"Spostrzeżenia",desc:"Automatyczne wnioski, dysproporcje i rekomendacje strategiczne",color:S.gold},
              ].map((s,i)=>(
